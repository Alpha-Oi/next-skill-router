# Deep dive: autopilot (nick-vels/skills)

**Source:** https://github.com/nick-vels/skills
**Analyzed:** 2026-09-21
**Path:** `\skills\autopilot`
**Method:** static analysis (см. `scripts/autopilot-deep-dive.mjs`)
...
| :--- | :--- |
| `name` | autopilot |
| `description` | Use when the user dictates an app, site, bot, or feature to build end-to-end and expects a finished result without reviewing specs, tickets, or code — vibecoding sessions, non-technical users, "собери |
...
## 3. Оглавление SKILL.md (11 заголовков)
...
- `phases/5-subagents`
- `phases/6-review`
- `phases/7-instruments`
- `phases/8-final`
- `phases/9-memory`
- `phases/5-repair`
- `phases/rationalizations`
- `phases/polish`
- `phases/0-instruments`
- `phases/0-memory`

## 5. Поддиректория `phases/` (если есть)

Найдено **17** файлов в `phases/`.
...
## 6. Ключевые принципы (по **bold** в body)

- **in one dialogue**
- **The order is the product.**
- **The brief is the contract, not the design.**
- **How far to take it is the user's dial**
- **detaches**
- **read at the moment that phase starts, not before**
- **One file at a time, and never ahead.**
- **Twice only where the table says twice**
- **After a compaction, re-read the state, not the phases.**
- **in Phase 4**
- **in Phase 5 and Phase 8**
- **exactly one Russian word per stage**
- **«Сборка» — это весь прогон, а не один этап.**
- **Единица работы — «таск».**
- **the mode**
- **the depth**
- **the finish**
- **the brief**
- **The rules for all three are in `phases/0-modes.md`, read in Phase 0 with `phases/0-preflight.md`**
- **no mode removes the manifest gates, and no mode removes the safety gates.**

## 7. Что можно взять в наш проект

_Заполняется вручную._

Гипотезы:

1. **Модульная структура `phases/`** — autopilot разбивает логику на этапы.
   Наш Composer (Phase 4) может использовать похожий подход: вместо одного
   большого DAG строить набор именованных этапов.
2. **`argument-hint` в frontmatter** — уже взято в v0.3.
3. **Длинное описание с триггерами** — autopilot использует очень подробное
   `description` (с русскими и английскими фразами). Это влияет на качество
   семантического матчинга.
4. **Self-contained подход** — «every rule it needs lives in `phases/`». Это
   перекликается с нашей идеей, что навык должен быть переносимым.

## 8. Полный frontmatter
